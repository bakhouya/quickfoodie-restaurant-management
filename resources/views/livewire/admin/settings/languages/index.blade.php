<div class="settings_wrapper_index">
    ...
    <div class="items_cards_settings">
        <div class="cards_flex">
            @forelse ($this->languages as $index => $lang)
                <div class="item_list">
                    <div class="header_item flex_center_center">
                        <div class="avatar_primary"> 
                            <img src="{{asset("storage/".$lang->image)}}" alt="">
                        </div>
                    </div>
                    <div class="slash"></div>
                    <div class="body_item flex_between_center">
                        <div class="flex_start_center gap_1">
                            <div class="text text_secoundary">{{$lang->name}}</div>
                            {!!helpHelper::status($lang->status)!!}
                        </div>
                        
                        <div class="flex_start_center actions_card">
                            <button type="button" class="text text_small" wire:click.stop='edit("{{$lang->id}}")'>
                            ...
                            <button type="button" class="text text_small" wire:click.stop='handleConfirmDelete("{{$lang->id}}")'>
                            ...
                            <div class="switch">
                                <input type="checkbox" hidden class="hidden"
                                    ...
                                    wire:change='handleChangeStatus({{$lang->id}})'
                                    id="status-{{$lang->id}}"/>
                                <label for="status-{{$lang->id}}" class="switch_primary"></label>
                            </div>
                        </div>
                    </div>
                </div>
