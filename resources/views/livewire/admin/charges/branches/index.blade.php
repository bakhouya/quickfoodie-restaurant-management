<div class="settings_wrapper_index">
    <div class="search_items">
        ...
        @livewire('admin.charges.branches.create')
    </div>
    <div class="items_cards_settings">
        <div class="cards_flex">
            @forelse ($this->branches as $index => $item)
            <div class="item_list">
                <div class="header_item flex_center_center">
                    <div class="avatar_primary sd"> <div class="text text_secoundary">{{$index}} </div> </div>
                </div>
                <div class="slash"></div>
                <div class="body_item flex_between_center">
                    <div class="">
                        <div class="flex_start_center gap_1">
                            <div class="text text_secoundary">{{$item->name}}</div>
                            {!!helpHelper::status($item->status)!!} 
                        </div>
                        <div class="flex_start_center mt_03">
                            <div class="text text_small">{{$item->city->name}}</div>
                            <div class="slash sm"></div>
                            <div class="text text_small">{{$item->phone}}</div>
                        </div> 
                    </div>
                    
                    <div class="flex_start_center actions_card">
                        <button type="button" class="text text_small" wire:click.stop='edit("{{$item->id}}")'>
                        ...
                        <button type="button" class="text text_small" wire:click.stop='handleConfirmDelete("{{$item->id}}")'>
                        ...
                        <div class="switch">
                            <input type="checkbox" hidden class="hidden"
                                ...
                                wire:change='handleChangeStatus({{$item->id}})'
                                id="status-{{$item->id}}"/>
                            <label for="status-{{$item->id}}" class="switch_primary"></label>
                        </div>
                    </div>
                </div>
            </div>
