<div wire:poll.alive.5s>
    ...
    @component('widgets.users.search')@endcomponent
    <section class="main_container">
        <article class="section_page" wire:ignore.self>
            ...
            <div class="container_cards active" wire:ignore.self>   
                ...
                @else
                    <div class="header_primary p_sticky">
                        <div class="flex_start_center gap_05">
                            <div class="text text_secoundary grey_text">personnel</div>
                            <div class="slash"></div>
                            <div class="text text_secoundary grey_text">
                                {{($this->users->currentPage() - 1) * $this->users->perPage() + 1}} 
                                for 
                                {{min($this->users->currentPage()*$this->users->perPage(),$this->users->total())}}
                                of 
                                {{$this->users->total()}}
                            </div>
                        </div>
                    ...
                    @else
                        <div class="grid_1 gap_secoundary cards_flex">
                            @forelse ($this->users as $user)
                                @component('widgets.users.user-card', [
                                "user"=>$user, 
                        ...
                        {{ $this->users->links('components.min.paginate') }}
                    @endif
                </article>
                {{-- =================boxcards================================= --}}
            </div> 
           
            @component('widgets.users.filter', [
                "selectRoleFilter" => $selectRoleFilter,
                "rolesFilter" => $this->roles,
                "textButtonGetFilterRole"=>$textButtonGetFilterRole ,
            ])@endcomponent
    ...
    @endif 
    @if ($selectRoleFilter)
        @component('widgets.users.select-role-filter',[
