...
class Index extends Component{
    ...
    public function assets(){
        return Asset::where('name', 'like', '%'. $this->search .'%')
                    ->orderBy('id', "DESC")->limit($this->limit)->get(); 
    }
    public function addLimit(){ 
        if (Asset::all()->count() > $this->limit) {
            $this->loadLimit = true ;
    ...
    public function handleChangeStatus(Asset $item){
    ...
    public function handleConfirmDelete(Asset $item){
        $this->delete = true ;
        $this->id = $item->id ;
    }
    public function forceDelete(){
        $item = Asset::where('id', $this->id)->first() ;
        if($item){
            $item->delete();
    ...
    public function render(){
        return view('livewire.admin.charges.forns.index');
    }
}
